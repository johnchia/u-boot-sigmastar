...
#define CONFIG_ARCH_INFINITY6C	1
 ...
 * assigned. Without this, env_flags gives it "mo" -- MAC, write-once -- and a
 * setenv over an existing one is refused with "Can't overwrite", which is
 ...
 * board but the first. The rootfs derives the real one from the NOR part's
 * unique ID (ethaddr_provision in /etc/init.d/rcS, reading /proc/flash_uid)
 * and writes it with fw_setenv, only ever when it is empty or still that
 * default.
 */
...
#if defined(CONFIG_MS_NOR_ONEBIN) && defined(CONFIG_MS_PARTITION)
